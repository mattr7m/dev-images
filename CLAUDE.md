...
Container images for Red Hat Devspaces / Eclipse Che CDE platform. Uses a base/derivative pattern — `images/base/Containerfile` extends the Red Hat Universal Developer Image (UBI9) with additional CLI tools and utilities.
...
```bash
podman build -t tools-udi -f images/base/Containerfile .
```

The build context must be the repo root (not `images/base/`) because the Containerfile copies files from `scripts/`.

## Architecture

- **`images/base/Containerfile`** — Main artifact. Installs CLI tools (argocd, gh, roxctl, kubeseal, kustomize, kubecolor, yq, colordiff, ansible) and the VSCode tunnel CLI on top of the UBI9 base image. Tool versions are pinned via `ARG` directives at the top of the file.
